...
<style>
	...
	<div class="row">
		<div class="col-sm-12">
			...
			<h3 class="heading-name">Generate Question Paper</h3>
			<div class="div-form">
				<form action="pdf_generate.php" method="POST">
					...
					<select id="subject_id" name="subject_id" required>
						...
						<?php if($subjects = fetchData(array('table'=>'subjects','condition'=>''))): ?>
							<?php foreach($subjects as $class): ?>
								<option value="<?php echo $class['subject_name']; ?>"><?php echo $class['subject_name']; ?></option>
							<?php endforeach; ?>
						<?php endif; ?>
					</select>
					...
					<select id="course_code" name="course_code" required>
						<option value="">Select the Subject</option>
						<?php if($course_codes = fetchData(array('table'=>'question_paper_formate','condition'=>''))): ?>
							<?php foreach($course_codes as $course_code): ?>
								<option value="<?php echo $course_code['course_code']; ?>"><?php echo $course_code['course_code']; ?></option>
							<?php endforeach; ?>
						<?php endif; ?>
					</select>
						...
						<?php $count=1; foreach($units as $unit): ?>
							<tr>									
								...
								<?php if($classs = fetchData(array('table'=>'class', 'condition'=>'WHERE id = '.$unit['class_id']))): ?>
									...
									<?php endforeach; ?>
								<?php endif; ?>
								<?php if($classs = fetchData(array('table'=>'semester', 'condition'=>'WHERE id = '.$unit['semester_id']))): ?>
									<?php foreach($classs as $class): ?>
								...
								<?php endif; ?>
								<td>PDF CODE HERE</td>
								<td><a href="/admin/delete_row.php?id=<?php echo $unit['id'].'&from=question_paper_list&calling_script='.$_SERVER['PHP_SELF']; ?>"><button style="background-color:red;" class="open-button">Delete</button></a></td>
							</tr>
